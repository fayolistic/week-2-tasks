...
// Create a copy of the input array to keep the original array
const inputCopy = input.slice();
 
function calculateAge(dob) {
  const latestDate = new Date('2019-01-01'); // Assume the current year is 2019
  const dateOfBirth = new Date(dob);
  const yearOfBirth = latestDate.getFullYear() - dateOfBirth.getFullYear();
  return yearOfBirth;
}
